...
<template>
  <UFileUpload v-model="files" accept="image/*" multiple>
    <template #actions="{ open }">
      <UButton label="Upload" icon="lucide:upload" color="neutral" variant="outline" @click="open()" />
    </template>
  </UFileUpload>
</template>
```
...
<!-- Flat array -->
<UDropdownMenu :items="[
  { label: 'Edit', icon: 'lucide:pencil', onSelect: () => edit() },
  { type: 'separator' },
  { label: 'Delete', icon: 'lucide:trash', color: 'error' }
]">
...
const groups = [{
  ...
  label: 'Actions',
  items: [
    { label: 'New file', icon: 'lucide:file-plus', onSelect: () => {} },
